...
- `cmd/server`: 服务入口，包含 `main` 函数。
- `internal/protocol`: GB/T 32960 协议解析核心逻辑 (支持 2016/2025)。
- `internal/infra`: 基础设施层，包含 MQ (RabbitMQ, Kafka) 实现。
- `internal/usecase`: 业务逻辑层，包含数据分发 (`dispatcher`) 和消息处理 (`handler`)。
